...
import { WeighingDetail } from '@/types/transaction';
...
export const WeightGrid = ({ weights, onUpdate, onDelete }: WeightGridProps) => {
    ...
    <div className="space-y-3">
      ...
      <div className="weight-grid grid grid-cols-5 gap-2 max-h-[40vh] overflow-y-auto p-1">
        {weights.map((weight, index) => (
          <div
            key={weight.id}
            className={`weight-item weight-item-enter relative group ${
              editingId === weight.id ? 'ring-2 ring-primary' : ''
            }`}
